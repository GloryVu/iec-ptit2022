...
# setting video scale
for i in range(num_of_camera):
    media_player_snapshot[i].video_set_scale(0.3)  
# start playing video
for i in range(num_of_camera):
    media_player_snapshot[i].play()

path = "D://iec_ptit_2022/cvanomaly_detection/record/image_data_set/"
presetName ="preset"
video_path = "D://iec_ptit_2022/cvanomaly_detection/record/video_data_set/"
#
is_preparing = [0]*num_of_camera
is_reset = [False]*num_of_camera
def reset_media_player_snapshot(iter):
    global is_reset
    global is_preparing
    if(media_player_snapshot[iter].is_playing() == 0 and is_reset[iter] == False):
        ...
        is_preparing[iter] = 20
        while(is_preparing[iter] > 0):
...
def snapshot(iter,preset):
    if(media_player_snapshot[iter].is_playing() == 1 and is_preparing[iter] == 0):
        media_player_snapshot[iter].video_take_snapshot(0, get_preset_path(iter,preset),1920,1080)  
    else:
        count = 0
        while(is_preparing[iter] > 0 and count < 30):
            time.sleep(1)
            count+=1
...
d1 = datetime.now()
d1 = datetime(d1.year,d1.month,d1.day,13,1,0)
